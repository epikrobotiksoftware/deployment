...
- For backup go to spawner by using mo-spawner command:
...
- navigate to the backup folder /home/{Robot_Name}/robots/{Robot_Name}/userspace/database/full_backup
- Go to packages folder and delete every folder except /maps /meshes /pointclouds
- create zip file of the whole fullbackup folder
- upload the zip file to the manager
- type `mo-spawner` to go to the docker container locate the fullbackup (`/opt/mov.ai/user/database/full_backup`
  ) directory and type the following command

```
python3.8 -m tools.backup -p full_backup  -a import -c -v -i
```
